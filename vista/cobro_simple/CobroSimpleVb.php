...
Phx.vista.CobroSimpleVb = {
	...
	title:'Pagos',
	nombreVista: 'CobroSimpleVb',
	
	constructor: function(config) {
        var me = this;
	   
	    Phx.vista.CobroSimpleVb.superclass.constructor.call(this,config);
        this.init();
    ...
    preparaMenu:function(n){
        ...
        if(this.historico == 'no'){
            ...
            if(data.estado=='borrador'&&data.codigo_tipo_cobro_simple!='PAG_DEV'){
                this.getBoton('btnAgregarDoc').enable();
            } else if(data.estado=='rendicion'){
                this.getBoton('btnAgregarDoc').enable();
            }

            if(data.estado=='rendicion'||data.estado=='vbconta'&&(data.codigo_tipo_cobro_simple=='PAG_DEV'||data.codigo_tipo_cobro_simple=='ADU_GEST_ANT'||data.codigo_tipo_cobro_simple=='SOLO_DEV'||data.codigo_tipo_cobro_simple==  'DVPGPR')){
                this.getBoton('btnAgregarDoc').enable();   
            }

            if(data.estado=='borrador'&&data.codigo_tipo_cobro_simple=='DVPGPR'){
                this.getBoton('btnAgregarDoc').enable();   
            }

            //Caso Paso ADU_GEST_ANT
            if(data.estado=='vbtesoreria'&&data.codigo_tipo_cobro_simple=='ADU_GEST_ANT'){
                this.getBoton('ant_estado').enable();
            }
        ...
        //if(data.estado=='rendicion'||data.estado=='vbconta'){
            if(data.codigo_tipo_cobro_simple=='DVPGPR'||data.codigo_tipo_cobro_simple=='SOLO_DEV'||data.codigo_tipo_cobro_simple=='ADU_GEST_ANT'||data.codigo_tipo_cobro_simple=='PAG_DEV'){
