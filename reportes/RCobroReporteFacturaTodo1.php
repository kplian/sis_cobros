...
class RCobroReporteFacturaTodo extends ReportePDF {
	...
	function generarCabecera(){
		$conf_par_tablewidths=array(7,20,20,25,40,25,15,25,25);
	...
	function imprimirLinea($val,$count,$fill){
		...
		foreach ($this->factura as $datarow) {
			...
			$saldo = $saldo - $cobroFactura ;				
						...
						//var_dump($newDateUP);
						$this->tablealigns=array('C','C','C','C','L','R','R','R','R');	// las alings de las tablas center left right		
						$this->tableborders=array('RLTB','RLTB','RLTB','RLTB','RLTB','RLTB','RLTB','RLTB','RLTB'); // los bordes de la tabla right left top botton
						...
						$RowArray = array(
							...
							's1' =>$newDate,
							's2' =>$newDateUP ,
							's3' =>$datarow['nro_documento'],
							's4' =>trim($datarow['razon_social']),
							's5' =>$datarow['importe_doc'],
							's6' =>$datarow['desc_moneda'],
							's7' =>$datarow['importe_cobrado_mb'],
							's8' =>	$datarow['saldo_por_cobrar'],			
						);
						$fill = !$fill;					
	...
	function cerrarCuadro(){		
		...
		$this->tableborders=array('T','T','T','T','T','LRTB','','LRTB','LRTB');	
