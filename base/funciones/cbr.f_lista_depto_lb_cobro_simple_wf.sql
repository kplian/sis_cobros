...
CREATE OR REPLACE FUNCTION cbr.f_lista_depto_lb_cobro_simple_wf (
...
***************************************************************************
 SCRIPT: 		cbr.f_lista_depto_lb_cobro_simple_wf
...
BEGIN
  v_nombre_funcion ='cbr.f_lista_depto_lb_cobro_simple_wf';

    --recuperamos la cotizacion a partir del id_estado_wf
    --validar de que sistema es el estado_wf
 
        select 
      dep.id_depto_origen as id_depto_lb,
      cd.id_depto_conta
    into
      v_registros_depto
    from cbr.tcobro_simple cd
   inner join param.tdepto_depto dep on dep.id_depto_destino = cd.id_depto_conta
   inner join param.tdepto d on d.id_depto = dep.id_depto_origen 
   inner join segu.tsubsistema sub on sub.id_subsistema = d.id_subsistema
    where cd.id_estado_wf =  p_id_estado_wf   and d.modulo = 'LB' and sub.codigo = 'TES'  ;
    ...
    IF not p_count then
              ...
              v_consulta:='SELECT 
                            ...
                            WHERE  DEPPTO.estado_reg = ''activo'' and
                               (
                                DEPPTO.id_depto in ('|| v_registros_depto.id_depto_lb::Varchar ||')
                               )
                               and '||p_filtro||'
